...
echo         self.drop_frame = ttk.LabelFrame(self.main_frame, text="Seleccionar imágenes", padding="20") >> "%SCRIPT_NAME%"
echo         self.drop_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10)) >> "%SCRIPT_NAME%"
echo         self.drop_label = ttk.Label(self.drop_frame, text="Haz clic aquí para seleccionar imágenes", justify=tk.CENTER) >> "%SCRIPT_NAME%"
...
echo         self.log("Listo para convertir imágenes. Haz clic en el área superior para seleccionar archivos.") >> "%SCRIPT_NAME%"
...
echo             self.drop_label.config(text="Haz clic aquí para seleccionar imágenes") >> "%SCRIPT_NAME%"
echo             return >> "%SCRIPT_NAME%"
